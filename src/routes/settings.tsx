      ...
      <div className="space-y-4 px-4 pt-4">
        ...
        <Card className="space-y-3 rounded-2xl p-4">
          ...
          <ToggleRow
            ...
            title='"Generated by Hisaab Kitaab" on invoices'
            subtitle="Show a small credit line at the bottom of every invoice PDF."
            checked={settings.invoiceWatermark !== false}
            onChange={(v) => updateSettings({ invoiceWatermark: v })}
          />
          <ToggleRow
            icon={<FileText className="h-4 w-4 text-primary" />}
            title='"Generated by Hisaab Kitaab" on statements'
            subtitle="Show a small credit line at the bottom of every statement PDF."
        ...
        <Card className="space-y-3 rounded-2xl p-4">
          <div className="flex items-center gap-2 text-sm font-semibold">
            <FolderOpen className="h-4 w-4 text-primary" /> Save folder (Android)
          </div>
          <p className="text-xs text-muted-foreground">
            Where PDFs and exports are saved on your device.
          </p>
          <div className="grid grid-cols-2 gap-2">
            {(["downloads", "documents"] as const).map(opt => (
              <button
                key={opt}
                onClick={() => updateSettings({ downloadDir: opt })}
                className={cn(
                  "rounded-xl border px-3 py-3 text-left text-sm font-medium capitalize transition-colors",
                  (settings.downloadDir || "downloads") === opt
                    ? "border-primary bg-primary/10 text-primary"
                    : "border-border hover:bg-accent",
                )}
              >
                <span className="block text-[10px] uppercase tracking-wider text-muted-foreground">
                  {opt === "downloads" ? "/Download/HisaabKitaab" : "/Documents/HisaabKitaab"}
                </span>
                {opt}
              </button>
            ))}
          ...
          {settings.downloadDir && (
            <Button variant="ghost" size="sm" className="h-8 text-xs text-muted-foreground"
              onClick={() => updateSettings({ downloadDir: undefined })}>
...
function ToggleRow({
  ...
  return (
    <button
      ...
      className="flex w-full items-center gap-3 rounded-xl border border-border bg-background/50 px-3 py-2.5 text-left transition-colors hover:bg-accent/40"
    >
